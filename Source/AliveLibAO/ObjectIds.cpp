...
void ObjectIds::Destructor()
{
    for (u32 i = 0; i < field_0_buffer_size; i++)
    {
        while (field_4_pBuffer[i])
        {
            ...
            ao_delete_free_447540(pCurrent);
        }
    }

    // Free the backing array
    ao_delete_free_447540(field_4_pBuffer);
}
...
{
    // Create new record
    ObjectId_Record* pRec = ao_new<ObjectId_Record>();
